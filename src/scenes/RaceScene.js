...
import { drawGoKart, drawRallyCar, drawMonsterTruck, drawFormulaOne, drawSupercar } from '../systems/CarRenderer.js';
...
export class RaceScene extends Phaser.Scene {
  ...
  _createCars() {
    ...
    const drawFn = drawFnMap[this.classId] || drawGoKart;

    // Get player customization from garage
    const progress = this.registry.get('progress');
    const customization = progress ? progress.getCarCustomization(this.classId) : { color: null, equipped: [] };
    const playerColor = customization.color || carColors[0];

    for (let i = 0; i < 4; i++) {
      const isPlayer = i === 0;
      const gfx = this.add.graphics().setDepth(isPlayer ? 5 : 3);
      const color = isPlayer ? playerColor : carColors[i];
      const y = lanes[i];
      ...
      // Draw equipped attachments for the player car
      if (isPlayer && customization.equipped && customization.equipped.length > 0) {
        for (const attachId of customization.equipped) {
          const attachDef = ATTACHMENTS.find(a => a.id === attachId);
          if (attachDef) attachDef.draw(gfx, color);
        }
      }
